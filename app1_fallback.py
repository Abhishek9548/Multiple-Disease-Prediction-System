...
st.markdown("""
    <style>
    .title { color: #1F77B4; font-weight: bold; text-align: center; }
...
def nav():
    if HAS_OPTION_MENU:
        with st.sidebar:
            st.image("https://static7.depositphotos.com/1007989/773/i/450/depositphotos_7735215-stock-illustration-health-is-wealth.jpg", width=120)
            return option_menu(
    ...
    with st.sidebar:
        return st.radio('Disease Prediction System', ['Diabetes Prediction', 'Heart Disease Prediction', 'Parkinsons Prediction'])
...
if selected == 'Diabetes Prediction':
    ...
    if st.button('Get Diabetes Test Result'):
        ...
        txt = 'The person is Diabetic 🩸' if pred == 1 else 'The person is not Diabetic 👍'
...
# --- Heart ---
    ...
    if st.button('Get Heart Disease Test Result'):
        ...
        txt = 'The person has Heart Disease 💔' if pred == 1 else 'The person does not have Heart Disease ❤️'
        st.markdown(f"<p class='result-text'>{txt}</p>", unsafe_allow_html=True)
...
# --- Parkinsons ---
    ...
    col1, col2, col3 = st.columns(3)
    fo = col1.slider('MDVP:Fo(Hz)', 50.0, 260.0, 120.0)
    fhi = col2.slider('MDVP:Fhi(Hz)', 100.0, 600.0, 200.0)
    flo = col3.slider('MDVP:Flo(Hz)', 50.0, 260.0, 110.0)
    ...
    if st.button("Get Parkinson's Test Result"):
        ...
        txt = "Likely Parkinson's 🧠" if pred == 1 else "Not likely Parkinson's 🌟"
        st.markdown(f"<p class='result-text'>{txt}</p>", unsafe_allow_html=True)
